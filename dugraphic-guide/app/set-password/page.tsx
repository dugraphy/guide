...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    // 초대 메일 링크를 거쳐 이 페이지로 오면 Supabase 클라이언트가
    // URL의 access_token을 자동으로 파싱해 세션을 발급한다.
  ...
  if (status === "checking") {
    return (
      <div className="flex flex-1 items-center justify-center min-h-screen">
        <p className="text-sm text-[var(--fg-muted)]">초대 링크를 확인하는 중입니다...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-1 items-center justify-center min-h-screen">
      <div className="w-full max-w-sm px-8 py-10 bg-[var(--bg)] border border-[var(--border)] rounded-xl shadow-sm">
        <h1 className="text-xl font-bold text-[var(--fg)] mb-6 text-center">비밀번호 설정</h1>
