...
const DEFAULT_NOTES: Record<QuoteType, string> = {
  리뉴얼:
    ...
    "진행 시스템\n" +
    "•  의뢰 접수 및 상담 → 계약금 입금 및 계약 체결 → 기존 홈페이지 자료 및 콘텐츠 확인 → 리뉴얼 시안 작업 및 검토 → 최종 검수 → 도메인/호스팅 파악 및 SEO 세팅 → 잔금 입금 후 결과물 제공 및 유지보수\n" +
    "계약 확정 및 홈페이지 제공\n" +
    "•  계약금은 시안 작업 비용이며, 계약금 입금 시 계약이 확정됩니다.\n" +
    "•  홈페이지 리뉴얼이 완료되면 잔금 입금 시 홈페이지가 제공됩니다.\n" +
    "검수 및 책임\n" +
    ...
    "•  단, 콘셉트/구조 미승인 상태에서 전체 재작업이 3회 이상 또는 부분 수정이 5회를 초과한 이후 계약을 진행하지 않는 경우 계약금의 50%만 환불되며, 두 조건이 동시에 해당하면 20%만 환불됩니다. (작업 기한 미준수, 반복적 요청 누락·오반영 등 수주인의 사정으로 계약 진행이 어려운 경우는 전액 환불)\n" +
    "•  의뢰인이 문자, 카카오톡, 이메일 등을 통해 시안에 대한 승인 의사를 명시적으로 전달한 이후에는 본 조항이 적용되지 않으며, 이후 절차는 「시안 승인 후 취소」 조항을 따릅니다.\n" +
    "•  환불은 요청일로부터 7일 이내 계좌이체로 지급합니다.\n" +
    "호스팅 서버 및 콘텐츠 사용\n" +
    "•  호스팅 서버 사용 계약 종료 시 홈페이지에 사용된 이미지, 사진 등 콘텐츠의 사용이 중단됩니다.\n" +
    "•  리뉴얼 이전의 기존 홈페이지 콘텐츠(이미지, 텍스트, 소스 등)는 의뢰인이 별도로 백업 및 보관하여야 하며, 백업하지 않아 발생한 손실에 대해서는 수주인이 책임지지 않습니다.\n" +
    "시안 승인 후 취소\n" +
    ...
    "•  각 시안 제공일로부터 3일 이내 수정 요청은 무상이며, 3일 경과 후 요청은 부분 수정 건당 5,000원(계약 진행 시 면제), 전체 재작업 건당 100,000원(진행 여부 무관 청구)이 발생합니다. 1차 시안 제출 이후 새로운 레퍼런스 요청은 전체 재작업으로 간주합니다.\n" +
  ...
  신규:
    ...
    "•  계약 전 계약서 및 서비스 약관을 반드시 정독해야 하며, 미숙지로 인한 책임은 의뢰인에게 있습니다.\n" +
    "진행 시스템\n" +
    "•  의뢰 접수 및 상담 → 계약금 입금 및 계약 체결 → 시안 작업 및 검토 → 최종 검수 → 도메인/호스팅 연결 및 SEO 세팅 → 잔금 입금 후 결과물 제공 및 유지보수\n" +
    "계약 확정 및 홈페이지 제공\n" +
    "•  계약금은 시안 작업 비용이며, 계약금 입금 시 계약이 확정됩니다.\n" +
